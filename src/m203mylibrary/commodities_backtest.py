...
from commodities_blockchain import generate_random_name
...
class RebalanceFlag:
    def time_to_rebalance(self, t: datetime, data_slice: pd.DataFrame) -> bool:
        """
        Returns True if it's time to rebalance. This is a base class.
        """
        return False
...
class ExpiryCheck(RebalanceFlag):
    ...
    def time_to_rebalance(self, t: datetime, data_slice: pd.DataFrame) -> bool:
        """
        Returns True if any contract in the data slice expires on date t.
        """
        if data_slice.empty:
            return False
        # Check if any row in data_slice has expiry_column == t.strftime('%Y-%m-%d')
        t_str = t.strftime('%Y-%m-%d')
        # Because expiry dates are in str form, compare
        expiring = data_slice[self.expiry_column] == t_str
        return expiring.any()
...
@dataclass
class CommodityStopLoss(CommodityRiskModel):
    threshold: float = 0.1
